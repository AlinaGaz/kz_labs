...
class ImageProcessingApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Image processing program")
    ...
    def create_widgets(self):
        ...
        self.load_button = tk.Button(self.controls_frame, text="Load img", command=self.load_image)
        self.load_button.pack(fill=tk.X, padx=5, pady=5)

        # Кнопка предварительной обработки
        self.preprocess_button = tk.Button(self.controls_frame, text="Image preprocessing", command=self.preprocess_image, state=tk.DISABLED)
        self.preprocess_button.pack(fill=tk.X, padx=5, pady=5)

        # Кнопка поиска контуров
        self.contours_button = tk.Button(self.controls_frame, text="Find the contours", command=self.find_contours, state=tk.DISABLED)
        ...
        self.save_button = tk.Button(self.controls_frame, text="Save the result", command=self.save_result, state=tk.DISABLED)
        self.save_button.pack(fill=tk.X, padx=5, pady=5)

        # Поле для настройки порогового значения
        self.threshold_label = tk.Label(self.controls_frame, text="Level:")
        ...
        self.min_area_label = tk.Label(self.controls_frame, text="Minimum contour area:")
        ...
        self.primitive_count_label = tk.Label(self.controls_frame, text="Number of primitives:")
    ...
    def find_contours(self):
        if self.processed_image is not None:
            ...
            self.display_image(image_with_contours, self.processed_canvas)
    ...
    def find_primitives(self):
        if self.processed_image is not None:
            ...
            self.display_image(self.processed_image, self.processed_canvas)
            self.primitive_count_value.config(text=f"triangle: {triangle_count}, quadrangle: {rectangle_count}, circle: {circle_count}")
