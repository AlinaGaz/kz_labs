...
class FeatureDetectionApp:
    def __init__(self, master):
        self.master = master
        self.master.title("Feature Detection Application")
        ...
        self.find_features_button = tk.Button(master, text="Find Features", command=self.find_features)
        self.find_features_button.grid(row=2, column=0, columnspan=2, pady=5)
        
        self.track_features_button = tk.Button(master, text="Track Features", command=self.track_features)
        self.track_features_button.grid(row=3, column=0, columnspan=2, pady=5)
        
        self.match_features_button = tk.Button(master, text="Match Features", command=self.match_features)
    ...
    # Method to display an image in a widget
    ...
    def track_features(self):
        ...
        if new_image_path:
            ...
            # Add debug output to check the dimensionality of st
            print("Dimensionality of st:", st.shape)

            good_indices = np.where(st==1)[0]  # Find indices of good points
            good_new = p1[good_indices]  # Choose good new points
            good_old = p0[good_indices]  # Choose corresponding old points
